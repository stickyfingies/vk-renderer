...
std::vector<char> GraphicsDevice::ReadFile(const char * fileName)
{
...
GraphicsDevice::GraphicsDevice(unsigned int width, unsigned int height)
	: currentFrame{ 0 }
	, width{ width }
	, height{ height }
...
VkSurfaceFormatKHR GraphicsDevice::ChooseSurfaceFormat(const std::vector<VkSurfaceFormatKHR> & availableFormats)
{
...
VkPresentModeKHR GraphicsDevice::ChooseSurfacePresentMode(const std::vector<VkPresentModeKHR> availablePresentModes)
{
...
void GraphicsDevice::CreateIndexBuffer(const void * data, unsigned int byteSize)
{
...
{
	if (validationLayersEnabled && CheckValidationLayerSupport() == false)
	{
...
VkShaderModule GraphicsDevice::CreateShaderModule(const std::vector<char> & code)
{
...
void GraphicsDevice::CreateVertexBuffer(const void * data, unsigned int byteSize)
{
	VkDeviceSize bufferSize = byteSize;
...
bool GraphicsDevice::DeviceSuitable(VkPhysicalDevice device)
{
...
uint64_t GraphicsDevice::FindMemoryType(unsigned int typeFilter, VkMemoryPropertyFlags properties)
{
...
void GraphicsDevice::SetupDebugCallback()
{
	if (validationLayersEnabled == false) return;
...
void GraphicsDevice::UpdateUniformBuffer(size_t currentImage)
{
