...
class VulkanApp {
  ...
  uint64_t FindMemoryType(unsigned int typeFilter,
...
public:
  VulkanApp(unsigned int width, unsigned int height);
  ...
  inline bool FramebufferResized() const { return framebufferResized; }

  inline void FramebufferResized(bool value) { framebufferResized = value; }
};
